...
def analysis_func(rdata:list[dict]):
    ...
    if crossTab.size == 0 or crossTab.shape[0] < 2 or crossTab.shape[1] < 2:
        return crossTab, "표번 자료가 부족해 카이제곱 검정 수핼 불가",df
    # 유의 수준 : 0.05
    a = 0.05
    ...
    if p >= a:
        results =f"p값 {p:.5f} >= {a} : 성별에 따라 커피 선호 브랜드는 <b>차이가 없다</b> {note}"
    else:
        results =f"p값 {p:.5f} <= {a} : 성별에 따라 커피 선호 브랜드는 <b>차이가 있다</b> {note}"
