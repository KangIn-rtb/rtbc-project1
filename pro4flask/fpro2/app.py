...
# request : 현재 들어온 HTPP 요청 정보(파라미터, 폼, 헤더, 쿠키...)를 담는 객체
# make_response : 응답 객체를 직접 만들어 반환할 때 사용하는 함수
...
@app.route("/login",methods=['GET', 'POST']) # 여기는 route에서 ~~한 방식들을 허용한다는 methods
def login():
    if request.method == 'GET': # GET 요청이면 / 여기는 method 방식은 == ~ 이기때문에 단수
        return """
            <h2>로그인 페이지</h2>
            <form method="post"> 
