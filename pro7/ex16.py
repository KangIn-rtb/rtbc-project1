...
print(c_r2) # 0.65784764

plt.scatter(x,y,label='초기 데이터')
plt.plot(x_fit,y_lin_fit,linestyle=':',label='liner fit(d=1),$R^2=%.2f$'%model_r2,c='b',lw=3)
plt.plot(x_fit,y_quad_fit,linestyle=':',label='quad fit(d=2),$R^2=%.2f$'%model_r2,c='r',lw=3)
plt.plot(x_fit,y_cubic_fit,linestyle=':',label='cubic fit(d=3),$R^2=%.2f$'%model_r2,c='k',lw=3)
plt.legend()
plt.show()
